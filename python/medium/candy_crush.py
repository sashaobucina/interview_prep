...
def candy_crush(board: List[List[int]]) -> List[List[int]]:
    """
    # 723: This question is about implementing a basic elimination algorithm for Candy Crush.
    ...
    (i, j) is empty. The given board represents the state of the game following the player's move.
    ...
    Now, you need to restore the board to a stable state by crushing candies according to the following rules:
        ...
        - After crushing all candies simultaneously, if an empty space on the board has candies on top 
            of itself, then these candies will drop until they hit a candy or bottom at the same time. (No new candies will drop outside the top boundary.)
    ...
    for col in range(n):
        ...
        for row in range(m - 1, -1, -1):
            if board[row][col] > 0:
                board[ptr_row][col] = board[row][col]
                ptr_row -= 1

        for row in range(ptr_row, -1, -1):
            board[row][col] = 0
...
if __name__ == "__main__":
    ...
    assert candy_crush(board) == new_board
